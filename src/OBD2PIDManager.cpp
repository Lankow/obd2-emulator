...
{
    addPID("Engine Speed", 0x010C, 2, 0.0f, 0.0f, 16383.75f, 100.0f, 100,
    ...
    addPID("Vehicle Speed", 0x010D, 1, 0, 0, 255, 1, 100);
}
...
OBD2PIDInfo *OBD2PIDManager::getPIDInfo(uint16_t pid) const
{
...
const std::pair<const uint16_t, std::unique_ptr<OBD2PIDInfo>> *OBD2PIDManager::getPIDInfoByIndex(uint8_t index) const
{
...
void OBD2PIDManager::addPID(std::string description, uint16_t pid, uint8_t length, double current, double min, double max, double increment, int pace,
                            std::function<int32_t(const double &)> customGetter)
{
    m_OBD2PIDInfoMap[pid] = std::unique_ptr<OBD2PIDInfo>(
        new OBD2PIDInfo(description, length, current, min, max, increment, pace, customGetter));
}

void OBD2PIDManager::addPID(std::string description, uint16_t pid, uint8_t length, double current, double min, double max, double increment, int pace)
{
    m_OBD2PIDInfoMap[pid] = std::unique_ptr<OBD2PIDInfo>(
        new OBD2PIDInfo(description, length, current, min, max, increment, pace, nullptr));
}